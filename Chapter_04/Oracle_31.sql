...
-- Calculate sum salaries inside each department
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, DEPARTMENT_ID, SALARY,
    SUM(SALARY) OVER (PARTITION BY DEPARTMENT_ID) AS DEPARTMENT_TOTAL_SALARY
    FROM EMPLOYEES;

-- Obtain the sum total of all salaries
SELECT SUM(SALARY) FROM EMPLOYEES;

SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, DEPARTMENT_ID, SALARY,
    SUM(SALARY) OVER (ORDER BY SALARY) AS TOTAL,
    SUM(SALARY) OVER (ORDER BY SALARY ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) "SUBSET"
    FROM EMPLOYEES ORDER BY SALARY;

SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, DEPARTMENT_ID, SALARY,
    SUM(SALARY) OVER (PARTITION BY DEPARTMENT_ID ORDER BY SALARY) AS SUM_SALARY_BY_DEPARTMENT,
    SUM(SALARY) OVER (PARTITION BY DEPARTMENT_ID ORDER BY SALARY ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS moving_average
    FROM EMPLOYEES;
